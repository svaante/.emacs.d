...
#+begin_src emacs-lisp
  (use-package window
    ...
    :config
    ;; Inspiration from https://stackoverflow.com/a/7939523
    (defun display-buffer-previous-selected-window (buffer alist)
      (let* ((buflist (buffer-list (selected-frame)))
             (buflist (seq-filter (lambda (b)
                                    (not (minibufferp b)))
                                  buflist))
             (buflist (delq (current-buffer) buflist))
             (winlist (mapcar 'get-buffer-window buflist))
             (winlist (delq nil winlist))
             (winlist (delq (selected-window) winlist)))
        (message "display-buffer-previous-selected-window")
        (when winlist (setq window (car winlist))
    ...
    (setq display-buffer-base-action
          '((display-buffer--maybe-same-window
             ...
             display-buffer-previous-selected-window
