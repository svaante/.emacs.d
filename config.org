...
#+begin_src emacs-lisp
  ...
  (defun company-pcomplete (command &optional _arg &rest ignored)
    ...
    (add-hook 'eshell-mode-hook '(lambda () (set (make-local-variable 'company-idle-delay) nil)))
    (add-hook 'eshell-mode-hook '(lambda () (set (make-local-variable 'company-backends) '(company-pcomplete company-files company-elisp))))
          ...
          (defun eshell-mode-configuration ()
            (with-eval-after-load 'evil-collection
              (+emacs/key-bindings)
              (define-key evil-normal-state-local-map "I" 'eshell-bol)
