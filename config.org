...
#+begin_src  emacs-lisp
  ...
  (defun generated-buffer-name-from (name)
    (save-match-data
      (string-match "\\(?1:.+\\)<\\(?2:[0-9]+\\)>\\|\\(?1:.+\\)" name)
      (let* ((base-name (match-string 1 name))
             (number (match-string 2 name))
             (parsed-number (if number (string-to-number number) 1))
             (new-number (number-to-string (1+ parsed-number))))
        (concat base-name "<" new-number ">" ))))

  (defun dtache-shell-command-tail-and-attach-session (session)
    "Tail 1000 and attach to SESSION with `async-shell-command'."
    (let* ((dtache--current-session session)
      ...
      (if (not (dtache--session-attachable session))
        ...
        (cl-letf* (((symbol-function #'set-process-sentinel) #'ignore)
                   ...
                   (dtache-log (dtache--session-file session 'log))
                   (command-prefix (concat "tail --lines 10000 " dtache-log " ; "))
                   (dtach-command (concat command-prefix (dtache-dtach-command session t))))
          (while (get-buffer-process buffer)
            (setq buffer (get-buffer-create (generated-buffer-name-from (buffer-name buffer)))))
  ...
  (use-package dtache
    ...
    (setq dtache-shell-command-session-action
          '(:attach dtache-shell-command-tail-and-attach-session
...
** Window lock
#+begin_src emacs-lisp
  (define-minor-mode window-lock-mode
    "Toggle window-lock mode.
  Interactively with no argument, this command toggles the mode.
  A positive prefix argument enables the mode, any other prefix
  argument disables it.  From Lisp, argument omitted or nil enables
  the mode, `toggle' toggles the state.

  When window-lock mode is enabled, the current window is
  dadicated to current buffer. When disabled remove window
  lock."
    :inital-value nil
    :global nil
    :lighter " 🔒"
    (set-window-dedicated-p (get-buffer-window) window-lock-mode))

  (evil-leader/set-key "wl" 'window-lock-mode)
#+end_src
** Fix color stuff
#+begin_src  emacs-lisp
