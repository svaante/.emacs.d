    ...
    (org-map-entries
     (lambda ()
       (when (and (org-entry-is-todo-p)
                  (not (plist-member (org-element--get-node-properties) :TIMESTAMP)))
         (org-set-property "timestamp" (format-time-string (car org-time-stamp-formats)))))))
...
** Window lock
#+begin_src emacs-lisp
  (defun set-window-buffer-lock ()
    (interactive)
    (setq-local mode-line-format (add-to-list 'mode-line-format
                                              '(:eval (when (window-dedicated-p) "LOCK "))))
    (set-window-dedicated-p (get-buffer-window) (not (window-dedicated-p)))
    (c-update-modeline))

  (evil-leader/set-key "wl" 'set-window-buffer-lock)
#+end_src
** Fix color stuff
#+begin_src  emacs-lisp
