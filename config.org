...
#+begin_src emacs-lisp
  (use-package emacs
    ...
    (evil-define-key '(normal insert) minibuffer-mode-map
      (kbd "C-.") #'consult-insert-directory)
...
** Shell-mode
#+begin_src emacs-lisp
  (defun term-char-turn-on ()
    ;; If process is killed error will be signaled
    (and-let* ((process (get-buffer-process (current-buffer)))
               ((process-live-p process)))
      (term-char-mode)))

  (defun term-line-turn-on ()
    ;; If process is killed error will be signaled
    (and-let* ((process (get-buffer-process (current-buffer)))
               ((process-live-p process)))
      (term-line-mode)))
  ...
  (use-package shell
    ...
    :hook (shell-mode
           . (lambda ()
               ...
               (require 'term)
               (add-hook 'evil-insert-state-entry-hook #'term-char-turn-on nil t)
               (add-hook 'evil-insert-state-exit-hook #'term-line-turn-on nil t)))
    :config
