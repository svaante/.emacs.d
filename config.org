...
** Package company
   #+BEGIN_SRC emacs-lisp
     (use-package company
       ...
       (setq company-icon-size '(auto-scale . 15)) ;; bigger then line height on osx
...
** Vertico, consult, embark
   #+begin_src emacs-lisp
     ...
     (use-package consult
       ...
       (defun +run-python ()
         "Run start python repl if not allready created and return the buffer."
         (run-python)
         (get-buffer "*Python*"))

       (defun +run-nodejs ()
         "Overide all of the internal pop to buffer stuff and just return the created buffer."
         (advice-add 'pop-to-buffer :override 'and)
         (advice-add 'nodejs-repl-mode :override 'and)
         (nodejs-repl)
         (advice-remove 'pop-to-buffer 'and)
         (advice-remove 'nodejs-repl-mode 'and)
         (with-current-buffer (get-buffer "*nodejs*")
           (nodejs-repl-mode))
         (get-buffer "*nodejs*"))

       (setq consult-buffer-comint-map '(("*Python*" . +run-python)
                                         ("*nodejs*" . +run-nodejs)))
