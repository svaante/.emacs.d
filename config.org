...
#+BEGIN_SRC emacs-lisp
  (use-package emacs
    ...
    :config
    (require 'consult-org)
    (setq consult-org-agenda-match "+LEVEL=1")
    (setq consult-org-agenda-scope '("~/org/notes.org"))

    ;; FIXME There is some bug happening with dedicated buffers still
    (defun consult-org-note ()
      "Jump to an Org heading."
      (interactive)
      (let* (orig-buffers
             ;; If current buffer is an indirect one lets keep it
             (org-last-indirect-buffer nil)
             (org-goto-marker-indirect
              (lambda (marker)
                ;; If current window is dedicated use last preview window
                (when (and (window-dedicated-p)
             ...
             (state (lambda (action candidate)
                      (cond
                       ;; Store all buffer state
                       ((eq action 'setup)
                        (setq orig-buffers (buffer-list)))
                       ;; Preview buffers
                       ((and (eq action 'preview)
                             candidate)
                        (funcall org-goto-marker-indirect candidate))
                       ;; Delete all new buffers
                       ((eq action 'exit)
                        (dolist (buffer (buffer-list))
                          (unless (member buffer orig-buffers)
                            (kill-buffer buffer))))
                       ;; Goto last candidate
        ...
        (consult--read
         (consult--slow-operation "Collecting headings..."
           (or (consult-org--headings nil consult-org-agenda-match consult-org-agenda-scope)
