...
#+begin_src emacs-lisp
  (use-package emacs
    :general
    ;; Have never got around to using M- bindings
...
#+begin_src emacs-lisp
  (use-package embark
    ...
    (setq embark-pre-action-hooks
     (assoc-delete-all 'eshell embark-pre-action-hooks))
...
Hook for inserting current region into minibuffer.
#+begin_src emacs-lisp
  (use-package minibuffer
    :hook
    (minibuffer-setup . minibuffer-insert-selection)
    :custom
    ;; Move cursor after inserting history
    (advice-add 'goto-history-element
                :after (lambda (&rest _) (end-of-buffer)))
    :preface
    ...
    (defun goto-history-element-advice-goto-eob (&rest _)
      (goto-char (point-max)))
    :config
    ;; Move point to end of buffer after `next-history-element' and
    ;; `previous-history-element'.
    (advice-add 'goto-history-element :after #'goto-history-element-advice-goto-eob))
#+end_src
...
#+begin_src emacs-lisp
  (use-package consult-dir
    :general
    ;; Insert directory
    (:states '(normal insert) (kbd "C-f") #'consult-insert-directory)
...
#+begin_src emacs-lisp
  (use-package shell
    ...
    (advice-add 'shell-command-sentinel :after #'async-command-sentinel-show-exit-code)
    ;; Bolt on char based input on evil insert mode,
    ;; `comint-send-input' breaks to often.
    (require 'term)
    (evil-set-auxiliary-keymap shell-command-mode-map 'insert (map-copy term-raw-map)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package dape
    ...
    (defun bury-window-by-buffer (buffer)
      (when-let ((window (get-buffer-window buffer)))
        (quit-window nil window)))
    :general
    (dape-global-map (kbd "H-i") #'dape-stack-select-down
    ...
    (add-hook 'dape-display-source-hook #'pulse-once)
    (add-hook 'dape-compile-hook #'bury-window-by-buffer)
    ;; java ughhh
    (require 'eglot)
...
#+begin_src emacs-lisp
  (use-package magit
    ...
    ;; FIXME: BURY-BUFFER DOES IS NOT A VALID CHOICE.
