...
** Vertico, consult, embark
   #+begin_src emacs-lisp
     ...
     (use-package vertico-repeat
       :load-path "./quelpa/build/vertico/extensions"
       :init
...
** Dtache
 #+begin_src  emacs-lisp
   (defun osx/dtache--add-end-of-session-notification-advice (session)
   ...
   (defun +dtache-get-last-session ()
     (car (seq-sort-by #'dtache--session-creation-time #'> (dtache--db-get-sessions))))

   (defun +dtache-delete-all ()
     (interactive)
     (dolist (session (dtache--db-get-sessions))
       (dtache-delete-session session)))

   (defun +dtache-projectile-shell-command ()
     (interactive)
   ...
   (defun +dtache-eshell-attach (session)
     ...
     (cl-letf* ((dtache--dtach-mode 'attach)
                ...
                (log-file (dtache-session-file session 'log))
                (input (concat (when log-file
                                 (format "cat %s && " log-file))
   ...
   If no input run the last shell command."
     (interactive
      (list
       (let* ((last-dtache-session (+dtache-get-last-session))
              (last-dtache-command (when last-dtache-session
                                     (dtache--session-command last-dtache-session)))
              (read-command (read-shell-command (concat "Dtache shell command"
                                                        (when shell-command-prompt-show-cwd
                                                          (format-message " in `%s'"
                                                                          (abbreviate-file-name
                                                                           default-directory)))
                                                        (when last-dtache-command
                                                          (format-message " (default: `%s')"
                                                                          last-dtache-command))
                                                        ": "
                                                        nil nil))))

           (if (string-empty-p read-command)
               last-dtache-command
             read-command))))
   ...
   (defun +dtache-open-last-session-dwim ()
     "Open last run dtache session"
     (interactive)
     (dtache-open-dwim (+dtache-get-last-session)))
   ...
   (use-package dtache
     ...
     (setq dtache-env "~/.emacs.d/quelpa/build/dtache/dtache-env")
     (setq shell-command-prompt-show-cwd t)

     (defvar embark-dtache-map (make-composed-keymap dtache-action-map embark-general-map))
     (add-to-list 'embark-keymap-alist '(dtache . embark-dtache-map))

     ;; Hack to get it to work on osx
     (when (eq system-type 'darwin)
       (advice-add 'dtache--add-end-of-session-notification :override #'osx/dtache--add-end-of-session-notification-advice))
     ...
     (evil-leader/set-key
       ...
       "do" '+dtache-open-last-session-dwim
       "dk" '+dtache-delete-all
       "pr" '+dtache-projectile-shell-command))
   ...
   (use-package dtache-eshell
     :quelpa (dtache-eshell :fetcher gitlab :repo "niklaseklund/dtache")
     :after dtache
     :hook (after-init . dtache-eshell-setup)
     :config
     (define-key dtache-action-map "e" #'+dtache-new-eshell-attach)

     (evil-define-key 'insert eshell-mode-map
       (kbd "S-<return>") 'dtache-eshell-create-session))
