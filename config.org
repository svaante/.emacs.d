         ...
         (setq-local completion-in-region-function
                     (lambda (&rest args)
                       (apply (if vertico-mode
                              ...
                              args)))))
    :config
    (defvar consult-sub-process-buffer-source
      `( :name     "Sub Processes"
         :hidden   t
         :narrow   ?s
         :category buffer
         ...
         :items    ,(lambda ()
                      (cl-loop for p in (process-list)
                               for b = (process-buffer p)
                               when (and b (process-query-on-exit-flag p))
                               collect (buffer-name b)))))
    (add-to-list 'consult-buffer-sources 'consult-sub-process-buffer-source 'append))
#+end_src
...
#+begin_src emacs-lisp
  (use-package shell
    :preface
    (defun shell-quick (&optional arg)
      "Create or switch to a shell buffer named *shell*<ARG>."
      (interactive "P")
      (shell (get-buffer-create
              (concat "*shell*" (when (numberp arg) (format "<%s>" arg))))))
    (defun shell-bang-bang-revert ()
      "Send !! to rerun last command."
      (interactive)
      (goto-char (point-max))
      (insert "!!")
      (comint-send-input))
    :general
    ;; (leader-map "r" 'shell-quick)
    (shell-mode-map :states 'normal "gr" 'shell-bang-bang-revert)
...
#+begin_src emacs-lisp
  ...
  (use-package xterm-color
    ...
    (advice-add #'compilation-filter
                :around #'xterm-color-advice-compilation-filter))
#+end_src

** Shell Command Menu - Run things again
#+begin_src emacs-lisp
