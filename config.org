...
#+begin_src emacs-lisp
  (use-package shell
    ...
    ;; Wait until there is output before displaying (we might not need
    ;; to display)
...
*** Gnus unread - Show unread in mode line
#+begin_src emacs-lisp
  (use-package gnus-unread
    :straight (gnus-unread :type git :host github
                           :repo "svaante/gnus-unread")
    :custom
    (gnus-unread-group "dpettersson.*Inbox")
    (gnus-unread-mode +1))
#+end_src
  ...
  (use-package mbsync
    ...
    (defun mbsync-init ()
      (if (executable-find "mbsync")
          (run-with-timer 0 mbsync-period #'mbsync)
        (error "No mbsync.")))
    :hook
    (after-init . mbsync-init)
    (mbsync . gnus-group-get-new-news))
#+end_src
