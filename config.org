...
#+begin_src emacs-lisp
  (use-package emacs
    :hook
    (prog-mode . (lambda () (setq indicate-buffer-boundaries 'left)))
    (text-mode . (lambda () (setq indicate-buffer-boundaries 'left))))
#+end_src
...
#+begin_src emacs-lisp
  (use-package which-key
    ...
    ;; Stop resizeing sidewindows in terminal
...
#+begin_src emacs-lisp
  ...
  (use-package project
    ...
    :config
    (advice-add
     #'project-current :around
     (defun project-current-ensure-tab (project-current &optional maybe-prompt directory)
       (when-let* ((project (funcall project-current maybe-prompt directory))
                   (tab-name (format "[%s]" (project-name project))))
         (prog1 project
           (when maybe-prompt
             (if-let* ((tab
                        (cl-find tab-name
                                 (funcall tab-bar-tabs-function nil)
                                 :key (lambda (alist) (alist-get 'name alist))
                                 :test #'equal)))
...
#+begin_src emacs-lisp
  (use-package consult-dir
    ...
    (defun consult-dired (directory)
      ...
      (interactive (list (consult-read-directory "Find directory: ")))
...
** Consult Shell Command - Run things again
#+begin_src emacs-lisp
