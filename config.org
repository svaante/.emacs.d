...
#+begin_src emacs-lisp
  (use-package shackle
    ...
    (setq shackle-rules
          '((shell-mode :noselect nil :align right :size 80 :dedicated t :popup t)
            (compilation-mode :noselect nil :align right :size 80 :dedicated t :popup t)
            ("*Help*" :select t :align right :size 75 :dedicated t :popup t)
            ("*devdocs*" :select t :align right :size 85 :dedicated t :popup t)
            ("\\*Messages\\*" :regexp t :select t :align below :size 0.30 :dedicated t :popup nil)
            ("\\*Embark Export Occur\\*" :regexp t :select t :align below :size 0.25 :dedicated t :popup t)
            ("*xref*" :select t :align below :size 0.25 :dedicated t :popup t)
            ("\\*Embark Export Grep\\*" :regexp t :select t :align below :size 0.25 :dedicated t :popup t)
            ("*Org Select*" :select t :align below :popup t)
            ("CAPTURE-.*\\.org" :regexp t :align below :size 0.40 :popup t)
            ("\\*Agenda Commands\\*" :regexp t :select t :size 0.25 :custom shackle-split-below)
            ("\\*Org Agenda\\*" :regexp t :select t :size 0.25 :custom shackle-split-below)  ;; this does not work for some reason
            ("\\*literate-calc\\*" :regexp t :select t :size 0.25 :dedicated t :custom shackle-split-below)
            ("\\*restclient\\*" :regexp t :select t :size 0.25 :custom shackle-split-below)
            ("\\*Python\\*" :regexp t :select t :size 0.25 :custom shackle-split-below)
  ...
  (defun project--read-file-cpd-relative-fix (prompt
    ...
    (let ((current-buffer-file-name buffer-file-name))
      (project--read-file-cpd-relative prompt
                                       all-files
                                       ;; Remove current buffer from candidates list
                                       (lambda (candidate)
                                         (not (string-suffix-p candidate current-buffer-file-name)))
                                       'project-file
                                       mb-default)))
...
#+begin_src emacs-lisp
  (advice-add 'read-shell-command :around
              (defun read-shell-command-default-wrapper (read-shell-command prompt &rest args)
                (let* ((insert (or (string-match-p ": $" prompt)
                                   (length prompt)))
                       (last (car shell-command-history))
                       (prompt
                        (concat
                         (substring prompt 0 insert)
                         (when last
                           (format " (%s)" last))
                         (substring prompt insert)))
                       (command (apply read-shell-command
                                       prompt
                                       args)))
                  (or (and (string-empty-p command)
                           last)
                      command))))
#+end_src
...
#+begin_src emacs-lisp
  (use-package dape
    ...
    (setq dape--cppdbg-command
          "~/.vscode/extensions/ms-vscode.cpptools-1.17.5-darwin-arm64/debugAdapters/bin/OpenDebugAD7")
    (setq dape--codelldb-server-command
          "~/.vscode/extensions/vadimcn.vscode-lldb-1.10.0/adapter/codelldb")
    (setq dape--cortex-path
          "~/.vscode/extensions/marus25.cortex-debug-1.12.0/")

    (setq dape-configs `((cppdbg
                          modes (c-mode c-ts-mode c++-mode c++-ts-mode)
                          command dape--cppdbg-command
                          command-cwd ,(file-name-directory dape--cppdbg-command)
                          :type "cppdbg"
                          :request "launch"
                          :cwd dape-cwd-fn
                          :program dape-find-file
                          :MIMode ,(if (eq system-type 'darwin) "lldb" "gdb"))
                         (codelldb
                          modes (c-mode c-ts-mode
                                 c++-mode c++-ts-mode
                                 rust-ts-mode rust-mode)
                          command dape--codelldb-server-command
                          host "localhost"
                          port 8080
                          command-args ("--port" "8080")
                          :type "lldb"
                          :request "launch"
                          :cwd dape-cwd-fn
                          :program dape-find-file)
                         ...
                         (cortex
                          modes (c-mode c-ts-mode)
                          command "node"
                          command-args ("dist/debugadapter.js")
                          command-cwd dape--cortex-path
                          :type "cortex-debug"
                          :request "launch"
                          :cwd dape-cwd-fn
                          :executable dape-find-file
                          :servertype "jlink"
                          :device "STM32F405VG"
                          :gdbPath "arm-none-eabi-gdb"
                          :gdbServerConsolePort 55878
                          :extensionPath dape--cortex-path
                          :interface "swd"
                          :swoConfig (:enabled nil)
                          :rttConfig (:enabled t
                                               :address "auto"
                                               :decoders [(:label ""
                                                                  :port 0
                          ...
                          :postLaunchCommands [])
                         (godot-launch
                          ;; Has some issues v4.1.1 sending several stopped events
                          modes (gdscript-mode)
                          host "127.0.0.1"
                          port 6006
                          :type "server"
                         ...
                         (vscode-js
                          ...
                          port 8123
                          command "node"
                          command-cwd "~/Workspace/vscode-js-debug/dist/"
