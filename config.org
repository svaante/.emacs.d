...
#+begin_src emacs-lisp
  (use-package emacs
    :custom
    (frame-title-format '(:eval (or buffer-file-name (buffer-name)))))
#+end_src
...
#+begin_src emacs-lisp
  (use-package embark
    ...
    ( :states '(normal emacs)
      (kbd "C-@")   'embark-act ;; In terminal C-@ -> {C-SPC,C-S-SPC}
      (kbd "C-SPC") 'embark-act)
    (embark-file-map "h" 'hexl-find-file))
#+end_src
...
#+begin_src emacs-lisp
  ...
  org-src-block actually calls `org-edit-src-code'."
      ...
      (cond ((and (buffer-narrowed-p)) (widen))
            ...
            ((derived-mode-p 'shell-mode) (shell-narrow-to-prompt))
...
#+begin_src emacs-lisp
  ...
  (use-package project
    :preface
    (defun project-switch-project-tab (dir)
      "Switch to project and it's tab."
      (interactive (list (funcall project-prompter)))
      (let* ((name
              (format "[%s]"
                      (file-name-nondirectory
                       (directory-file-name dir))))
             (index (tab-bar--tab-index-by-name name)))
        (cond
         (;; No project tab, create it
          (not index)
          (tab-bar-switch-to-tab name)
          (project-switch-project dir))
         (;; Otherwise *just* switch
          t (tab-bar-select-tab (1+ index))))))
    ...
    (defun project-read-file-relative-w/o-current (prompt all-files &rest args)
      (apply #'project--read-file-cpd-relative prompt
             ...
             args))
    :general
    (leader-map "SPC" 'project-find-file)
    (project-prefix-map
     "t" 'project-switch-project-tab
     "C" 'project-recompile
     "e" 'project-async-shell-command
     "g" 'consult-ripgrep)
    :custom
  ...
  (use-package project-watch
    :after project
    :vc (:url "https://github.com/svaante/project-watch.el")
    :init
...
#+begin_src emacs-lisp
  (use-package frame
    :general
    (leader-map "w" '(:keymap window-prefix-map :which-key "windows and frames"))
    (window-prefix-map
     "f" 'tear-off-window
...
#+begin_src emacs-lisp
  (use-package simple
    ...
    ;; Install exit code on `mode-line-process'.
    (advice-add
...
** Shell - Also run things
#+begin_src emacs-lisp
  (use-package shell
    :preface
    (defun shell-revert-buffer ()
      (interactive)
      (goto-char (max-char))
      (insert "!!")
      (comint-send-input))
    :general
    (shell-mode-map :states '(normal) "gr" #'shell-revert-buffer)
    (shell-mode-map :states '(insert)
    ...
    (defun compilation-notify (_buffer message)
      (when (eq system-type 'gnu/linux)
        ...
        (notifications-notify
         :title "Compilation"
         :body (format "Status `%s'" (string-trim-right message)))))
    :hook
...
#+begin_src emacs-lisp
  ...
  (use-package consult-lsp
    :after lsp-mode
    :vc (:url "https://github.com/svaante/bear-cache")
    :ensure t
    :general
...
#+begin_src emacs-lisp
  (use-package acpe
    :vc (:url "https://github.com/svaante/acpe")
    :general
    (ctl-x-map
     "C-r" 'acpe
     "@" 'acpe-insert))
...
#+begin_src emacs-lisp
  (use-package magit
    ...
    :general
    ("M-m" '(:which-key "magit")
     "M-m m" 'magit-status-here
     "M-m f" 'magit-find-file
     "M-m d" 'magit-diff
     "M-m b" 'magit-blame
     "M-m l" 'magit-log
     "M-m c" 'magit-checkout)
    :custom
    ...
    ;; Visit editable files rather then read only buffers
...
#+begin_src emacs-lisp
  (use-package dired
    ...
    (dired-mode-map
     ...
     "q" 'kill-current-buffer)
    ;; Repeat Mode map clashing with evil
    (dired-jump-map "j" nil)
    :hook
