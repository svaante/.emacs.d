...
#+begin_src emacs-lisp
  ...
  (setq split-width-threshold (* 90 2))
...
#+begin_src emacs-lisp
  ...
  (use-package emacs
    ...
    (evil-define-key '(normal insert) global-intercept-mode-map
      ...
      "\C-ws" 'split-follow-window-vertically
      "\C-wv" 'split-follow-window-horizontally))
#+END_SRC
* Looks
** Basics
#+begin_src emacs-lisp
  ...
  (defun mode-line-file-path-1 (file-name)
    (propertize
     (abbreviate-file-name
      (if-let* ((project (project-project-root))
                (project-parent
    ...
    (advice-add 'shell-command-sentinel :after
                (defun shell-command-sentinel-show-exit-code (process status)
                  (when-let* ((buffer (process-buffer process))
                    ...
                    (let ((dead (memq (process-status process) '(exit signal))))
                      (with-current-buffer buffer
...
#+begin_src emacs-lisp
  (use-package dape
    ...
    (add-hook 'dape-active-mode-hook (lambda ()
